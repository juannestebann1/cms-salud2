...
class EntradaModel extends DataAccessLayer
{
	...
	public function apt()
	{
		...
		{
			$entradas = array();
			foreach ($this->Link->query("
							SELECT * FROM apartamentos RIGHT JOIN edificios ON apartamentos.id_edificio=edificios.id_edificio
						") as $row) {
		        $entradas[] = (object) $row;
		    }
	...
	public function FotoApt($id)
	{
		...
		try 
		{
		    ...
		    $sql = "SELECT * FROM fotoapartamento WHERE id_apartament = $id";
		    foreach ($this->Link->query($sql) as $row) {
		        $entradas[] = (object) $row;
		    }
